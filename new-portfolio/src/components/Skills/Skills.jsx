...
const skills = [
  {
    name: "Scrum",
    logo: "https://img.icons8.com/?size=100&id=ZyFMUVbtNQiG&format=png&color=000000",
  },
  {
    name: "React Native",
...
const Skills = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-fit bg-[#b63231] pt-12 lg:pt-[2.4rem] lg:pb-20 px-4 pb-12">
      {/* Section Title */}
      <h1
        ref={textRef}
        data-type="text"
        className={`text-3xl xs:text-3xl sm:text-4xl lg:text-5xl text-white font-extrabold tracking-widest text-center pb-6 lg:pb-3 lg:pt-6 transition-all duration-500 ${
          isTextVisible
            ? "animate-slideInFromTop opacity-100 translate-y-0"
      ...
      <p
        className={`text-base text-gray-300 sm:text-lg lg:text-xl text-center px-4 lg:px-20 pb-6 lg:pb-4 transition-all duration-500 ${
          isTextVisible
            ? "animate-slideInFromTop opacity-100 translate-y-0"
            : "opacity-0 translate-y-10"
        }`}
      ...
      >
        {skills.map((skill, index) => (
          <div
            key={skill.name}
            data-index={index}
            className={`flex flex-col items-center justify-center bg-[#353535] p-2 rounded-2xl shadow-lg transition-transform transform hover:scale-105 hover:shadow-2xl ${
